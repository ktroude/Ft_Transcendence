<script lang=ts>
  ...
  async function handleSubmit(event) {
    ...
    if (accessToken) {
      const response = await fetch(`http://${process.env.LOCALHOST}:3000/${user.id}/auth/2fa/verify`, {
        method: 'POST',
          headers: {
      ...
      if (data.isVerified)
      {
        const response = fetch(`http://${process.env.LOCALHOST}:3000/${user.id}/auth/2fa/lockstatus`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`

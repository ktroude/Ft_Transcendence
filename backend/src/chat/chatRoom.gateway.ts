...
export class ChatRoomGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleDisconnect(client: Socket) {
    // trouver le pelo qui se deco et le virer du tableau clients si il existe
      const index = this.clients.findIndex(([, socket]) => socket === client);
      if (index !== -1) {
        this.clients.splice(index, 1);
      }
      // il faudra aussi gerer les chatRooms quand le proprio se deco
      // changer le proprio avec un modo? ou juste supp la room? a voir
    }
    ...
    // on ajoute le createur aux membres et aux admins
    // peut etre pas aux admins a voir (question de privileges)
    ...
    async handleNewAdmin(@ConnectedSocket() client : Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const newAdmin = await this.userService.findUserByPseudo(data.newAdminPseudo);
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
      if (await this.chatRoomService.isAdmin(user, chatRoom) === 
    ...
    async handleUnAdminded(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
      ...
      const userToUnadmin = await this.userService.findUserByPseudo(data.userToUnadmined);
      if (await this.chatRoomService.isAdmin(userToUnadmin, chatRoom) === false ||
    ...
    async handleNewMuted(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
    ...
    async handleUnMuted(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
      const userToUnmute = await this.userService.findUserByPseudo(data.userToUnmute);
      if (await this.chatRoomService.isAdmin(user, chatRoom) === 
      await this.chatRoomService.isOwner(user, chatRoom) === 
      false) {
        // checker si le user est un admin ou le owner
    ...
    async handleNewBan(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
      ...
      if (await this.chatRoomService.isAdmin(userToBan, chatRoom) === true &&
    ...
    async handleUnBan(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
      const user = this.clients.find(([, socket]) => socket === client)?.[0];
      const chatRoom = await this.prismaService.chatRoom.findUnique(data.chatRoomId);
      const userToUnban = await this.userService.findUserByPseudo(data.userToUnban);
      if (await this.chatRoomService.isAdmin(user, chatRoom) === 
      await this.chatRoomService.isOwner(user, chatRoom) === 
      false) {
        // checker si le user est un admin ou le owner
      ...
      if (await this.chatRoomService.isBanned(userToUnban, chatRoom) === false) {
